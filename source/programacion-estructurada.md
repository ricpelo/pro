...
- La **pila de control** es una estructura de datos que utiliza el intérprete
  para llevar la cuenta de las funciones activas en un determinado momento,
  incluyendo el valor de sus parámetros y variables locales, y el punto de
  retorno al que debe devolverse el control cuando finalice la ejecución de la
  función.

- La pila es, básicamente, un **almacén de entornos**.
...
!DOT(pila-control.svg)(Pila de control en la línea 15)(width=100%)(width=60%)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
rankdir = LR
compound = true
node [shape = record]
...
subgraph cluster1 {
    ...
    node [fontname = "monospace"]
    30 [shape = circle]
    sesenta1 [shape = circle, label = "60"]
    x [shape = plaintext, fillcolor = transparent]
    u [shape = plaintext, fillcolor = transparent]
    x -> 30
    u -> sesenta1
}
subgraph cluster2 {
    ...
    bgcolor = "white"
    node [fontname = "monospace"]
    sesenta2 [shape = circle, label = "60"]
    y [shape = plaintext, fillcolor = transparent]
    y -> sesenta2
}
subgraph cluster3 {
    ...
    bgcolor = "white"
    node [fontname = "monospace"]
    w [shape = plaintext, fillcolor = transparent, label = ""]
}
subgraph cluster0 {
    label = "Marco global"
    bgcolor = "white"
...
30 -> g [lhead = cluster0, ltail = cluster1]
pila:f1 -> x [lhead = cluster1]
pila:f2 -> y [lhead = cluster2]
pila:f3 -> w [lhead = cluster3]
sesenta2 -> g [lhead = cluster0, ltail = cluster2]
w -> g [lhead = cluster0, ltail = cluster3]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
